Ext.ns('BugOver.user');
  ...
  * @extends Ext.Panel
  */
BugOver.user.UserPanel=Ext.extend(Ext.Panel,{
	 layout:'border',
	 iconCls:'icon-group',
     initComponent:function(){
          ...
          BugOver.user.UserPanel.superclass.initComponent.call(this);
    ...
    var store = new Ext.data.JsonStore({
          //url: BugOver.path+'/user/findAll.do',
          idProperty: 'userID',
          autoDestroy:true,
          root:'root',
          data:{root:[{userNo:'100001',name:'张三',isValid:'Y',telephone:'13560023432',roles:'系统管理员'},
          			  {userNo:'100002',name:'李四',isValid:'Y',telephone:'13560023432',roles:'库房管理员'}]},
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        bbar:new BugOver.UserPagingToolbar({
	             store:store,
	             pageSize:30}),
        columns:[sm,{header : '编号',dataIndex: 'userNo'},
                 {header : '名称',dataIndex: 'name'},
                 {header : '电话',dataIndex: 'telephone'},
                 {header : '是否有效',dataIndex: 'isValid',renderer:function(v){
                     if (v == 'Y') {
                 ...
                 {header : '角色',dataIndex: 'roles',width:150},
               {
                xtype: 'actioncolumn',
                items: [{
                    iconCls : 'delete',
...
Ext.reg('userpanel',BugOver.user.UserPanel);

/**
 * @namespace BugOver.user
 * @class BugOver.user.UserWindow
 * @extends Ext.Window
 * add and edit user window
 */
BugOver.user.UserWindow = Ext.extend(Ext.Window, {
			width : 400,
			height : 300,
			layout : 'fit',
			type:'add',
			buttonAlign:'center',
			initComponent : function() {
				BugOver.user.UserWindow.superclass.initComponent.call(this);
				var me = this;
				/**
				 * user form
				 */
...
/**
 * @namespace BugOver.user
 * @class BugOver.user.SelectUserWin
 * @extends Ext.Window
 */
BugOver.user.SelectUserWin=Ext.extend(Ext.Window,{
   width:400,
   height:400,
   layout:'fit',
   buttonAlign:'center',
   initComponent:function(){
       BugOver.user.SelectUserWin.superclass.initComponent.call(this);
