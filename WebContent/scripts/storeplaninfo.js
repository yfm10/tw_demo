Ext.ns('BugOver.storeplaninfo');

 /**
  * @namespace BugOver.storeplaninfo
  * @class BugOver.storeplaninfo.StoreplaninfoPanel
  * @extends Ext.Panel
  */
BugOver.storeplaninfo.StoreplaninfoPanel=Ext.extend(Ext.Panel,{
	 layout:'border',
     ...
     initComponent:function(){
          ...
          BugOver.storeplaninfo.StoreplaninfoPanel.superclass.initComponent.call(this);
    ...
    delRecord : function(record) {
		...
		if (length > 0) {
			...
			Ext.Msg.confirm('删除用户', '确定删除当前选择用户?', function(btn) {
						if (btn == 'yes') {
							Ext.Ajax.request({
										url : BugOver.path + '/storeplaninfo/delStoreplaninfos.do',
										params : {
											userIDs : ids
										},
    ...
    showAddWin:function(){
    	var me=this;
    	var win = new BugOver.storeplaninfo.StoreplaninfoWindow({
    		                type:'add',
    		                title:'添加用户',
							buttons : [{
										text : '确定',
										handler : function(btn) {
											me.saveStoreplaninfo(btn, win,'add');
										},
										scope : this
									}, {
										text : '取消',
									...
									Ext.EventManager.on(cm.getEl(), {
										keyup : function(e) {
											if (Ext.EventObject.ENTER == e.getKey()) {
												me.saveStoreplaninfo(win.buttons[0], win,'add');
											}
										},
										scope : me
									}, me);
    ...
    /**
     * save storeplaninfo to database
     * @param {Button} btn
     * @param {StoreplaninfoWindow} win
     */
    saveStoreplaninfo : function(btn, win,type) {
				var form = win.form.form;
				if (form.isValid()) {
					btn.disable();
					form.submit({
								url : BugOver.path+'/storeplaninfo/'+type+'.do',
        ...
        if(record){
            var win = new BugOver.storeplaninfo.StoreplaninfoWindow({
							...
							buttons : [{
										text : '确定',
										handler : function(btn) {
											me.saveStoreplaninfo(btn, win,'update');
										},
										scope : this
									}, {
										text : '取消',
    ...
    var store = new Ext.data.JsonStore({
          //url: BugOver.path+'/storeplaninfo/findAll.do',
          idProperty: 'userID',
          autoDestroy:true,
          root:'root',
...
Ext.reg('storeplaninfopanel',BugOver.storeplaninfo.StoreplaninfoPanel);

/**
 * @namespace BugOver.storeplaninfo
 * @class BugOver.storeplaninfo.StoreplaninfoWindow
 * @extends Ext.Window
 * add and edit storeplaninfo window
 */
BugOver.storeplaninfo.StoreplaninfoWindow = Ext.extend(Ext.Window, {
			width : 400,
			height : 300,
			layout : 'fit',
			type:'add',
			buttonAlign:'center',
			initComponent : function() {
				BugOver.storeplaninfo.StoreplaninfoWindow.superclass.initComponent.call(this);
				var me = this;
				/**
				 * storeplaninfo form
				 */
			...
			/**
			 * create storeplaninfo formpanel
			 *
			 * @method
			 */
			createForm : function() {
...
/**
 * @namespace BugOver.storeplaninfo
 * @class BugOver.storeplaninfo.SelectStoreplaninfoWin
 * @extends Ext.Window
 */
BugOver.storeplaninfo.SelectStoreplaninfoWin=Ext.extend(Ext.Window,{
   width:400,
   height:400,
   layout:'fit',
   buttonAlign:'center',
   initComponent:function(){
       BugOver.storeplaninfo.SelectStoreplaninfoWin.superclass.initComponent.call(this);
   ...
   getSelectedStoreplaninfoIDs:function(){
