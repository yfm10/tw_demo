Ext.ns('BugOver.authsys');

 /**
  * @namespace BugOver.authsys
  * @class BugOver.authsys.AuthSysPanel
  * @extends Ext.Panel
  */
BugOver.authsys.AuthSysPanel=Ext.extend(Ext.Panel,{
     ...
     initComponent:function(){
          ...
          BugOver.authsys.AuthSysPanel.superclass.initComponent.call(this);
    ...
    delRecord : function(record) {
		...
		if (length > 0) {
			...
			Ext.Msg.confirm('删除用户', '确定删除当前选择用户?', function(btn) {
						if (btn == 'yes') {
							Ext.Ajax.request({
										url : BugOver.path + '/authsys/delAuthSyss.do',
										params : {
											userIDs : ids
										},
    ...
    showAddWin:function(){
    	var me=this;
    	var win = new BugOver.authsys.AuthSysWindow({
    		                type:'add',
    		                title:'添加用户',
							buttons : [{
										text : '确定',
										handler : function(btn) {
											me.saveAuthSys(btn, win,'add');
										},
										scope : this
									}, {
										text : '取消',
									...
									Ext.EventManager.on(cm.getEl(), {
										keyup : function(e) {
											if (Ext.EventObject.ENTER == e.getKey()) {
												me.saveAuthSys(win.buttons[0], win,'add');
											}
										},
										scope : me
									}, me);
    ...
    /**
     * save authsys to database
     * @param {Button} btn
     * @param {AuthSysWindow} win
     */
    saveAuthSys : function(btn, win,type) {
				var form = win.form.form;
				if (form.isValid()) {
					btn.disable();
					form.submit({
								url : BugOver.path+'/authsys/'+type+'.do',
    ...
    var store = new Ext.data.JsonStore({
          //url: BugOver.path+'/authsys/findAll.do',
          idProperty: 'userID',
          autoDestroy:true,
          root:'root',
...
Ext.reg('authsyspanel',BugOver.authsys.AuthSysPanel);

/**
 * @namespace BugOver.authsys
 * @class BugOver.authsys.AuthSysWindow
 * @extends Ext.Window
 * add and edit authsys window
 */
BugOver.authsys.AuthSysWindow = Ext.extend(Ext.Window, {
			width : 400,
			height : 300,
			layout : 'fit',
			type:'add',
			buttonAlign:'center',
			initComponent : function() {
				BugOver.authsys.AuthSysWindow.superclass.initComponent.call(this);
				var me = this;
				/**
				 * authsys form
				 */
...
/**
 * @namespace BugOver.authsys
 * @class BugOver.authsys.SelectAuthSysWin
 * @extends Ext.Window
 */
BugOver.authsys.SelectAuthSysWin=Ext.extend(Ext.Window,{
   width:400,
   height:400,
   layout:'fit',
   buttonAlign:'center',
   initComponent:function(){
       BugOver.authsys.SelectAuthSysWin.superclass.initComponent.call(this);
   ...
   getSelectedAuthSysIDs:function(){
